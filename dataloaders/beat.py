...
class CustomDataset(Dataset):
    def __init__(self, args, loader_type, augmentation=None, kwargs=None, build_cache=True):
        ...
        self.use_seed_pose = args.use_seed_pose
    ...
    def cache_generation(self, out_lmdb_dir, disable_filtering, clean_first_seconds,  clean_final_seconds, is_test=False):
        ...
        for pose_file in pose_files:
            ...
            if self.use_seed_pose:
                logger.info(colored(f"# ---- Use seed pose for Pose   {id_pose} ---- #", "blue"))
                seed_pose = "/mnt/disk1/xey/audio2pose/datasets/seeds/output.bvh"
                with open(seed_pose, "r") as seed_pose_data:
                    for j, line in enumerate(seed_pose_data.readlines()):
...
#                     audio_each_file = audio_each_file[::sr//16000]
                    ...
                    # 获取需要处理的帧数，向下取整
                    frames = int(audio_each_file.size / 16000 * 15)
                except:
            ...
            if self.use_seed_pose:
                logger.info(colored(f"# ---- Use seed face for Facial   {id_pose} ---- #"))
                seed_face_file = "/mnt/disk1/xey/audio2pose/datasets/seeds/2_scott_0_1_1.json"
                with open(seed_face_file, "r") as facial_data_file:
                ...
                try:
                    ...
                    # continue
                    logger.info("The JSON file is empty, using zero padding.")
                    facial_data = {}
                    facial_each_file.append(facial_data)
                    facial_each_file = np.array(facial_each_file)
            ...
            if not pose_each_file.any():
                logger.info("sample from clip audio base...")
                filtered_result = self._sample_from_clip_audio_base(
                    dst_lmdb_env,
    ...
    def _sample_from_clip(
        ...
        round_seconds_audio = len(audio_each_file) // self.audio_fps
        ...
        max_round_seconds = max(round_seconds_audio, round_seconds_skeleton, round_seconds_facial)
        ...
        # if round_seconds_skeleton < max_round_seconds:
        #     pose_each_file = np.pad(pose_each_file, ((0, (max_round_seconds - round_seconds_skeleton) * self.pose_fps), (0, 0)), 'constant')
        #     logger.info(f"padding pose to {max_round_seconds}s")
        # if round_seconds_facial < max_round_seconds:
        #     facial_each_file = np.pad(facial_each_file, ((0, (max_round_seconds - round_seconds_facial) * self.pose_fps), (0, 0)), 'constant')
        #     logger.info(f"padding facial to {max_round_seconds}s")
            ...
            if len(sample_pose_list) > 0:
                with dst_lmdb_env.begin(write=True) as txn:
                    for pose, audio, facial, word, vid, emo, sem in zip(sample_pose_list,
                        ...
                        # if self.use_seed_pose:
                        #     normalize_face = self.normalize_face(facial, self.mean_facial, self.std_facial)
    ...
    def _sample_from_clip_audio_base(
        self, dst_lmdb_env, audio_each_file, pose_each_file=None, facial_each_file=None, word_each_file=None,
        vid_each_file=None, emo_each_file=None, sem_each_file=None,
        disable_filtering=False, clean_first_seconds=0, clean_final_seconds=0, is_test=False,
        ):
        """
        for data cleaning, we ignore the data for first and final n s
        for test, we return all data 
        ...
        if pose_each_file:
            pose_start = int(self.alignment[1] * self.pose_fps)
            pose_each_file = pose_each_file[pose_start:]
            round_seconds_skeleton = pose_each_file.shape[0] // self.pose_fps  # calculate skeleton duration
        else:
            round_seconds_skeleton = float('inf')  # set to a very large number if pose data is not present

        round_seconds_audio = len(audio_each_file) // self.audio_fps # calculate audio duration

        if facial_each_file.any():
            round_seconds_facial = facial_each_file.shape[0] // self.pose_fps # calculate facial duration
            logger.info(f"audio: {round_seconds_audio}s, pose: {round_seconds_skeleton}s, facial: {round_seconds_facial}s")
            round_seconds_skeleton = min(round_seconds_audio, round_seconds_skeleton, round_seconds_facial)
            max_round = max(round_seconds_audio, round_seconds_skeleton, round_seconds_facial)
            if round_seconds_skeleton != max_round: 
        ...
        clip_s_f_audio, clip_e_f_audio = self.audio_fps * clip_s_t, clip_e_t * self.audio_fps

        # 计算 pose 的长度
        clip_s_f_pose, clip_e_f_pose = clip_s_t * self.pose_fps, clip_e_t * self.pose_fps
        ...
        for ratio in self.multi_length_training:
            ...
            logger.info(f"pose from frame {clip_s_f_pose} to {clip_e_f_pose}, length {self.pose_length}")
            # else:
            #     num_subdivision = math.floor((clip_e_f_audio - clip_s_f_audio - self.pose_length) / self.stride) + 1
            #     logger.info(f"audio from frame {clip_s_f_audio} to {clip_e_f_audio}, length {self.pose_length}")
            ...
            pose_each_file = np.zeros((self.pose_length, 141), dtype=float)
            if not facial_each_file.any():
                facial_each_file = np.zeros((self.pose_length, 51), dtype=float)
            for i in range(num_subdivision):
                ...
                sample_word = word_each_file[start_idx:fin_idx] if word_each_file is not None else np.array([-1])
                sample_emo = emo_each_file[start_idx:fin_idx] if emo_each_file is not None else np.array([-1])
                sample_sem = sem_each_file[start_idx:fin_idx] if sem_each_file is not None else np.array([-1])
                sample_vid = np.array(vid_each_file) if vid_each_file is not None else np.array([-1])
                
                if sample_pose is not None and sample_pose.any() != None:
                    sample_pose, filtering_message = MotionPreprocessor(sample_pose, self.mean_pose).get()
                    is_correct_motion = (sample_pose != [])
                    if is_correct_motion or disable_filtering:
                        sample_pose_list.append(sample_pose)
            ...
            if len(sample_audio_list) > 0:
                with dst_lmdb_env.begin(write=True) as txn:
                    for pose, audio, facial, word, vid, emo, sem in zip(sample_pose_list,
                        ...
                        normalized_face = self.normalize_face(facial, self.mean_facial, self.std_facial)
                        # normalized_face = facial
    ...
    @staticmethod
    def normalize_face(dir_vec, mean_face, std_face=None):
        return (dir_vec + mean_face) / std_face
